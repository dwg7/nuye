...
6. トップバーの「写真」ボタンで、GSIシームレス空中写真(kitaphoto17、北海道)の
   表示を切り替えられる。ベースマップとは独立にon/offできる
7. トップバーの「地形」ボタンで、陰影起伏・3D地形の表示を切り替えられる
   (`hfu/mapterhorn-japan-bridge`)。これも写真・ベースマップと独立
8. 線(経路)を選択すると、属性パネルに「標高断面を見る」ボタンが現れ、
   簡易的な標高断面(距離・標高レンジ)を確認できる
9. サイドバー「サンプルミッションを読込」で、札幌駅〜月寒中央の模擬調査経路・
...
- 空中写真: 国土地理院シームレス空中写真(GSI seamlessphoto、CC BY 4.0)を、
  stars経由のkitaphoto17(`https://stars.optgeo.org/kitaphoto17`)として直接参照する
